...
int main() {
    ...
    while (1) {
        ...
        // 1) compute new iterate in parallel (each i independent)
        #pragma omp parallel for default(none) shared(a,b,x,x_courant,n) schedule(static)
        for (int i = 0; i < n; i++) {
            double s = 0.0;
            ...
            x_courant[i] = (b[i] - s) / a[i * n + i];
        }

        // 2) compute absmax with reduction(max:)
        double absmax = 0.0;
        #pragma omp parallel for default(none) shared(x,x_courant,n) reduction(max:absmax) schedule(static)
        for (int i = 0; i < n; i++) {
        ...
        // 3) copy x <- x_courant (parallel copy)
        #pragma omp parallel for default(none) shared(x,x_courant,n) schedule(static)
        for (int i = 0; i < n; i++)
            x[i] = x_courant[i];
    }
